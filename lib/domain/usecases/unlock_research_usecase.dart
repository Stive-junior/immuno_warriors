...
import 'package:immuno_warriors/domain/repositories/user_repository.dart'; // Pour gérer les points de recherche

class UnlockResearchNodeUseCase {
  final ResearchService _researchService;
  final UserRepository _userRepository; // Assumant que les points de recherche sont gérés par l'utilisateur
  ...
  Future<void> execute(String nodeId) async {
    try {
      ...
      final int userResearchPoints = userResources['researchPoints'] as int? ?? 0;
      ...
      final researchNode = await _researchService.getResearchTree().then((nodes) => nodes.firstWhere((node) => node.id == nodeId));
      if (researchNode != null) {
        final updatedResources = {'researchPoints': userResearchPoints - researchNode.cost};
