...
final signUpUseCaseProvider = Provider<SignUpUseCase>((ref) {
  ...
  return SignUpUseCase(
    ...
    userRepository,
    networkInfo,
  ); // Passe NetworkInfo au constructeur
});
...
final checkSessionValidityUseCaseProvider =
    Provider<CheckSessionValidityUseCase>((ref) {
      final userRepository = ref.read(userRepositoryProvider);
...
final clearUserSessionUseCaseProvider = Provider<ClearUserSessionUseCase>((
  ref,
) {
  final userRepository = ref.read(userRepositoryProvider);
...
final getUserResourcesUseCaseProvider = Provider<GetUserResourcesUseCase>((
  ref,
) {
  final userRepository = ref.read(userRepositoryProvider);
...
final updateUserResourcesUseCaseProvider = Provider<UpdateUserResourcesUseCase>(
  (ref) {
    final userRepository = ref.read(userRepositoryProvider);
...
final getUserInventoryUseCaseProvider = Provider<GetUserInventoryUseCase>((
  ref,
) {
  final userRepository = ref.read(userRepositoryProvider);
...
final updateUserInventoryUseCaseProvider = Provider<UpdateUserInventoryUseCase>(
  (ref) {
    final userRepository = ref.read(userRepositoryProvider);
    final authService = ref.read(authServiceProvider);
...
final updateUserSettingsUseCaseProvider = Provider<UpdateUserSettingsUseCase>((
  ref,
) {
  final userRepository = ref.read(userRepositoryProvider);
...
final getUserProgressionUseCaseProvider = Provider<GetUserProgressionUseCase>((
  ref,
) {
  final userRepository = ref.read(userRepositoryProvider);
...
final updateUserProgressionUseCaseProvider =
    Provider<UpdateUserProgressionUseCase>((ref) {
      final userRepository = ref.read(userRepositoryProvider);
...
final updateUserAchievementsUseCaseProvider =
    Provider<UpdateUserAchievementsUseCase>((ref) {
      final userRepository = ref.read(userRepositoryProvider);
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> _checkInitialSession() async {
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        final sessionValid = await _checkSessionValidityUseCase.execute(
          currentUser.id,
        );
        if (sessionValid) {
          state = state.copyWith(
            user: currentUser,
            isLoading: false,
            sessionExpiry: DateTime.now().add(
              const Duration(days: 7),
            ), // Supposons une validité de 7 jours pour la session
            errorMessage: null,
          );
          AppLogger.info(
            'Initial session check: User ${currentUser.email} session is valid.',
          );
        } else {
          // Session non valide, efface la session locale et l'état
          await _clearSession(currentUser.id);
          AppLogger.info(
            'Initial session check: User ${currentUser.email} session expired or invalid. Cleared session.',
          );
        }
      } else {
        state = state.copyWith(
          isLoading: false,
          errorMessage: null,
        ); // Aucun utilisateur trouvé, pas d'erreur
        AppLogger.info('Initial session check: No current user found.');
      }
    } catch (e, stackTrace) {
      AppLogger.error(
        'Error during initial session check',
        error: e,
  ...
  Future<bool> authenticateUser(String email, String password) async {
    ...
    try {
      ...
      if (isValid) {
        ...
        final user =
            await _getCurrentUserUseCase
                .execute(); // Cela devrait récupérer l'utilisateur authentifié
        if (user != null) {
          state = state.copyWith(
            user: user,
            isLoading: false,
          ...
          AppLogger.warning(
            'Authentication successful for $email, but UserEntity could not be retrieved.',
          );
          return false;
        }
      }
      ...
      state = state.copyWith(
        ...
        errorMessage:
            AppStrings
                .invalidPassword, // Ou un message plus spécifique si l'API le permet
      );
      AppLogger.warning(
        'Authentication failed for $email: Invalid credentials.',
      );
      return false;
    } catch (e, stackTrace) {
      AppLogger.error(
        'Error during authentication for $email',
        error: e,
        stackTrace: stackTrace,
      );
      state = state.copyWith(isLoading: false, errorMessage: e.toString());
  ...
  Future<void> signUp({
    ...
    try {
      ...
      if (user != null) {
        ...
        state = state.copyWith(
          isLoading: false,
          errorMessage: AppStrings.registerFailed,
        );
  ...
  Future<void> signOut() async {
    ...
    try {
      ...
      state = state.copyWith(
        isLoading: false,
        errorMessage: AppStrings.signOutFailed,
      );
    }
  }
  ...
  Future<void> _fetchCurrentUser() async {
    try {
      ...
      AppLogger.error(
        'Error fetching current user in AuthNotifier',
        error: e,
        stackTrace: stackTrace,
      );
  ...
  Future<bool> checkSessionValidity() async {
    ...
    if (!state.isSignedIn || !state.isSessionValid) {
      final currentUserId = state.user?.id;
      if (currentUserId != null) {
        final isValid = await _checkSessionValidityUseCase.execute(
          currentUserId,
        );
...
final userListProvider = FutureProvider<List<UserEntity>>((ref) async {
  ...
  if (!authState.isSignedIn) {
    AppLogger.info(
      'userListProvider: User not signed in, returning empty list.',
    );
    return [];
  }
  try {
    ...
    AppLogger.error(
      'Error fetching user list in provider',
      error: e,
      stackTrace: stackTrace,
    );
...
final userResourcesProvider = FutureProvider.autoDispose<Map<String, dynamic>>((
  ref,
) async {
...
final userInventoryProvider = FutureProvider.autoDispose<List<dynamic>>((
  ref,
) async {
  final userId = ref.watch(authProvider.select((state) => state.user?.id));
  if (userId != null) {
...
final userSettingsProvider = FutureProvider.autoDispose<Map<String, dynamic>>((
  ref,
) async {
  final userId = ref.watch(authProvider.select((state) => state.user?.id));
  if (userId != null) {
...
final userAchievementsProvider = FutureProvider.autoDispose<Map<String, bool>>((
  ref,
) async {
  final userId = ref.watch(authProvider.select((state) => state.user?.id));
  if (userId != null) {
